...
int main(int argc,char *argv[])
{
  ...
  CC=      12.0;        		/* halo concentration      */
  Mvir=   150.0;         	/* virial mass (in 10^10 Msolar) */
  ...
  H=      3.2;          	/* manually set radial disk scale length, this decouples
                           		the angular momentum of the disk and the halo and hence
                           		the spin doesn't set the size of H and Jd is not
                           		fixed.   */
  DiskHeight=  0.075;    	/* thickness of disk in units of radial scale length */
  ...
  HoleRadius = 0.00;			/* in units of Rd, when inner gas disk is cut off */
  ...
  BulgeSize=   1.0;    			/* bulge scale length in ABSOLUTE UNITS (*NOT* units of R_d) */
