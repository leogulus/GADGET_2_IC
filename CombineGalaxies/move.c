...
//      semimajor = sqrt( rmin * rmin / (1 - ecc*ecc) );
//
//  printf("semimajor     = %g\n",semimajor);
//  printf("1-ecc*ecc     = %g\n",(1-ecc*ecc));
//  printf("numerator     = %g\n",(semimajor*(1-ecc*ecc)/Rstart - 1.0));
//  printf("argument      = %g\n",(semimajor*(1-ecc*ecc)/Rstart - 1.0)/ecc);
//

//      phistart = -acos( (semimajor*(1-ecc*ecc)/Rstart - 1.0)/ecc );

//    }
//  else
//    {
//      phistart = -acos(p/Rstart - 1);
//    }
  ...
  printf("p             = %g\n",p);
  printf("phistart      = %g\n",phistart);
  printf("sin(phistart) = %g\n",sin(phistart));
  printf("rdotstart     = %g\n",rdotstart);
  printf("phidotstart   = %g\n",phidotstart);
  printf("r*phidotstart = %g\n",rstart*phidotstart);

#ifdef ECC_NONZERO
//   if(ecc<1) rmax= p/(1-ecc);  /* REMOVED BECAUSE THIS MAKES IT FAIL FOR E<1!!!! */
//  if(ecc<0) rmax= p/(1-ecc);
//  printf("Rstart=%g\nrmax=%g\t",Rstart,rmax);
//  if(Rstart>rmax)
//    {
